...
class Grammar
{
    ...
    recogTYPE expectedError( recogTYPE type, char* info );  //返回一个缺少的语法错误
    recogTYPE varNotExist( char * token );                  //返回一个未定义的常量/变量/过程的错误
    recogTYPE dupliDef(char *type, char *token);            //返回一个重复定义的错误
